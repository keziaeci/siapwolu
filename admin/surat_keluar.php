<?php 
    ...
    $jumlahData = count(query("SELECT * FROM surat_keluar")); 
    $jumlahHalaman = ceil($jumlahData / $jumlahDataPerHalaman); //membulatkan keatas
    $halamanAktif = (isset($_GET["halaman"])) ? $_GET["halaman"] : 1; 
    $awalData = ($jumlahDataPerHalaman * $halamanAktif) - $jumlahDataPerHalaman;

    $surat = query("SELECT * FROM surat_keluar ORDER BY id DESC LIMIT $awalData, $jumlahDataPerHalaman ");
...
<head>
    ...
    <title>Surat Keluar - Admin</title>
    ...
    <!--Container Main start-->
      <div class="col-12">
        <div class="row">
          <div class="col mt-4">
            <h3 class="fw-semibold">Daftar Surat Keluar</h3>
          </div>
        </div>
            <div class="card shadow-2">
              <div class="card-header">
                <div class="row">
                  <a  class="col-sm-2 btn text-light bg-primary" href="tambah_sk.php"><b>+</b>  Tambah Surat Keluar</a>
                ...
                <div class="card-body p-1 m-0 table table-responsive">
                    <table class="table  table-striped display nowrap w-100  aos-init aos-animate" border="5" cellpadding="10" cellspacing="0">
                        <tr>
                            <th>No. Urut</th>
                    ...
                    <?php foreach($surat as $row) : ?>
                        <tr>
                          ...
                          <td><?= $row["keperluan"]; ?></td>
                          <td>
                            <a class="btn btn-success" href="../page/prev_sk.php?id=<?= $row['id']; ?>" target="_blank">Preview</a>
                          </td>
                          <td>
                          <a class="btn btn-warning" href="ubah_sk.php?id=<?= $row["id"]; ?>">Ubah</a>
                          <a class="btn btn-danger" href="../page/hapus_sk.php?id=<?= $row["id"]; ?>" onclick  ="return confirm('Apakah anda yakin?');">Hapus</a>
                          </td>
                          <?php if($row["laporan"] === "Tidak ada") : ?>
                              <td>
                                <p>tidak ada</p>
                              </td>
                              <?php else: ?>
                              <td>
                                <a class="btn btn-success" href="../page/prev_lpr.php?id=<?= $row['id']; ?>" target="_blank">Preview</a>
                              </td>
                            <?php endif; ?>
                        </tr>
                        <?php $i++; ?>
